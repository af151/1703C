		...
		<div class="row">
			...
			<div class="col-md-7 split min_h_500">
				...
				<c:choose>
						...
						<div class="media">
						  <div class="media-left">
						    <a href="/article?id=${article.id}">
						      <img class="media-object" src="${pageContext.request.contextPath }${article.picture}" width="120" alt="${article.title }">
						    </a>
						  </div>
						  <div class="media-body">
						    <h3 class="media-heading article_title" style="${article.style}"><a href="/article?id=${article.id}">${article.title }</a></h3>
						    <p>${article.summary}</p>
					...
					<c:otherwise>
						  <div>
						    <h3><a class="article_title" href="/article?id=${article.id}" style="${article.style}">${article.title }</a></h3>
						    <p>${article.summary}</p>
						    <p class="blog_item_footer">
						    	<span class="glyphicon glyphicon-user" title="作者"></span>作者：${article.author.nickname}&nbsp;&nbsp;&nbsp;
						    	<span class="glyphicon glyphicon-time" title="发布时间"></span>发布：&nbsp; <fmt:formatDate value="${article.updated}" pattern="yyyy-MM-dd HH:mm:ss"/>&nbsp;&nbsp;&nbsp;&nbsp; 
			...
			<!-- 最新图片文章 -->
				<div class="card">
					...
					<div class="card-body">
						<ol>
							<c:forEach items="${picArticlesList}" var="article">
							<li class="text-truncate"><a href="/article?id=${article.id}">${article.title}</a></li>
							</c:forEach>
						</ol>
					</div>
